...
const route = useRoute(); const tenant = useActiveTenant(); const headers = useStorefrontHeaders()
const { data, error, status, refresh } = await useFetch<any>(`/api/v1/storefront/pages/${route.params.slug}`, { headers })
useSeoMeta({ title: () => data.value?.data ? `${data.value.data.title} — ${tenant.value?.brand?.identity?.name || ''}` : (tenant.value?.brand?.identity?.name || ''),
